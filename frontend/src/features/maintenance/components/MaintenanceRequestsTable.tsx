...
export function MaintenanceRequestsTable({ filters, onNavigateToTeams, onNavigateToEquipment }: MaintenanceRequestsTableProps) {
  ...
  const requests = useMemo(() => {
    let filtered = allRequests.filter((r) => r.isActive)
    ...
    if (filters?.search) {
      ...
      filtered = filtered.filter((r) => {
        const equipmentName = getEquipmentName(r.equipmentId).toLowerCase()
        const technicianName = getTechnicianName(r.technicianId).toLowerCase()
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-100">Maintenance Requests</h2>
      ...
      <MaintenanceRequestModal
        ...
        request={allRequests.find((r) => r.id === selectedRequestId)}
        mode="edit"
